...
| `IN-009` | HIGH | Gus | Route the **second full-set rater** for `S1-ENDORSE-V2` Part A: 1,080 rows, a frontier model from a **different family** than Claude. The package is on Torch at `/scratch/gs157/marin-misinfo-labels/endorsement_feature_v1_rater_upload/` — 10 parts of 108 rows, each in `.jsonl`, `.csv` and `.md`, plus the frozen `PROMPT.md`. Preferred: an API key on Torch, since 1,080 rows through a web UI is ten long paste sessions. Web-UI routing works too, one part at a time. Rater A (Claude, full set) is done and sealed; a second Claude pass would measure test-retest, not rater robustness, which is the whole point of this task. Return files as `labels_part<N>.jsonl`. | The `S1-ENDORSE-V2` disagreement audit and every checkpoint reading | Two sealed rater files passing `scripts/check_endorsement_labels.py` |
