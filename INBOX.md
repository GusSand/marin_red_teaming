...
| `IN-009` | HIGH | Gus | Route the **second full-set rater** for `S1-ENDORSE-V2` Part A to **GPT** (or another non-Claude frontier model). The package is built and verified at `/scratch/gs157/marin-misinfo-labels/endorsement_feature_v1_raterB_gpt/` — read its `README.md` first. Send `upload/PROMPT.md` plus one part's items; 10 parts of 108. Each part ships as `.jsonl`, `.csv` and `.md` — pick what the interface accepts. Return either `labels_part<N>.jsonl` or a filled `sheet_part<N>.csv`; both paths are dry-run tested end to end. Preferred: an API key on Torch, since 1,080 rows through a web UI is ten long sessions. Rater A (Claude) is sealed; a second Claude pass would measure test–retest, not rater robustness, which is the whole point of this task. The earlier `_rater_upload/` directory is superseded — do not send it. | The `S1-ENDORSE-V2` disagreement audit and every checkpoint reading | Two sealed rater files passing `scripts/check_endorsement_labels.py` |
| `IN-010` | LOW | Gus | Heads-up, no action needed unless you disagree. The frozen `S1-ENDORSE-V2` plan names its intervals "P0 and P1 ... following `S1-STATS`" but describes P1 as the two-way cluster bootstrap, which is `S1-STATS`'s **P2**. Rather than pick after the fact I report P0, P1 and P2 and decide on the **widest** — weaker than either reading, so it cannot be selection on the outcome. Same for "Holm-adjusted intervals": significance now needs the widest interval to exclude zero **and** Holm-adjusted p < 0.05. Both declared in the experiment file before any label was viewed. | Nothing | `docs/experiments/09-10_endorsement-feature_decomposition.md` § Declared readings |
| `IN-008` | HIGH | Gus | Approve adopting `P1` (seed-level t) as this project's interval procedure, or direct a successor experiment to pre-register a **one-sided** acceptance bar. The frozen bar in `S1-STATS` was two-sided, `[2.0%, 12.0%]`, which fails a procedure for being too conservative as well as too liberal. `P1` lands at 4.8-7.1% in six of eight cells and fails only on one cell at 0.0%. The incumbent `P0` reaches 40.5%. Re-running with a friendlier bar after seeing these rates would be selecting on the outcome, so the agent will not do it unprompted. | `S1-STATS` close; the interval labelling on every recorded result | A decision line in `docs/decisions.md` |
<!-- ACTIVE_INBOX_END -->
...
- **[2026-08-29 08:40] DECISION NEEDED — neither local judge cleared the pre-registered thresholds against the Claude anchor** (qwen72 stance F1 0.66, relevance 0.37; olmo32 unusable). Per the rule I did not pick one. Three ways forward, my recommendation first: **(a) use the Claude annotator itself as the judge on the full 1,080 set**, audited by your 25-item spot-check (`annotate.py` on `calibration_v1/spotcheck`, ~15 min) — no threshold relaxation, one rater throughout, limitation stated; (b) collapse the `partial` classes into neighbours as rubric v2 and re-select (qwen would likely pass on stance, still marginal on relevance) — a declared deviation; (c) more human annotation (the pre-registered path, but your time). Step 3 waits on this.

- **[2026-08-29 01:20] Step 4 wrapper result corrected after verification — pre-registered criterion NOT met.** I first reported +22pp [+10, +32] for the wrapper × checkpoint interaction. The verifier found 13–21 null WildGuard labels per raw-continuation file (1–2-char outputs) that my proxy counted as attempts. Treated as non-responses: **−5.6pp [−17.9, +8.6]**, criterion fails. Scaffolded-only, post hoc: +12.3 [0.0, +22.8]. Everything is in the experiment doc and a journal CORRECTION entry. Step 1 (IFEval +11.8pp) is unaffected and verified. No decision needed unless you want the wrapper step re-run with more seeds.

- **[2026-08-29 08:25] Annotation plan changed per your call: a blind Claude subagent labels the 150; you spot-check ~25 disagreement items** (`annotate.py` on `calibration_v1/spotcheck`, ready once the anchor sheet lands). The two items below are superseded.
- **[2026-08-29 00:45, update] The two judges disagree badly (stance agreement 0.40; OLMo calls 634/1080 refusals, Qwen 180, WildGuard ~220). Your 150 labels are now the only thing that decides which, if either, is usable. Step 3 is blocked on them.**
- **[2026-08-29] Human calibration annotation needed (Stage 1, step 2) — ~150 items, yours.** Blinded set at `/scratch/gs157/marin-misinfo-labels/calibration_v1/`: read `items.jsonl` (cid, request, response), fill `sheet.csv` (relevance / task / stance / specificity / coherence / persuasiveness) using the locked rubric `config/judge_rubric_v1/`. **Do not open `key.json`** (checkpoint identity). Quality columns blank when task = no_attempt. Two candidate judges (Qwen2.5-72B-Instruct, OLMo-2-32B-Instruct, both ungated) are downloading; selection runs against your sheet via `scripts/compare_judges.py`. If a second annotator is available for 50 items, say who.

- **[2026-08-28] Two Stage 2 questions for David** (Stage 2 doc is blocked on them; Stage 1 proceeds meanwhile, spec `docs/experiments/08-28_phoenix-starling_distribution-decomposition.md`). (1) **Which intermediate Starling checkpoints still exist** (steps / token fractions, storage path)? They set whether a 10% futility rule is justified and give the timing profile. (2) **Is the cluster budget sufficient** for the six-arm screen at the Starling token budget, two training/data-order seeds on A–D, and one confirmation run of whichever removal arm (E/F) matters — 10 runs, plus one? If not, which arms to drop first.

- **[2026-08-28] CORRECTION, no answer needed — I overclaimed "instruction-following is ruled out for the cooldown".** Only the empty/echo component is excluded. Off-topic was never detected, and the data are consistent with it: compliant-but-unharmful responses fall 21.8% → 11.9% Phoenix→Starling. WildGuard cannot separate off-topic from on-topic-but-weak, so the +11pp harmful|non-refusal is H1+H1b unsplit. Fixed in journal (appended), spec, report, vault. H0 / H-min / mixed verdicts unaffected. Consequence: the relevance / gradable-benign-task instrument is needed for the cooldown claim, not just the early tags — it should go on the BACKLOG ahead of the ablation one-pager.
...
## 2026-09-09 · IN-007 RESOLVED — no token needed, again

WildGuard was restored from `/scratch/gs157/safety-decay/hf_cache/hub/models--allenai--wildguard`, the
same route it arrived by on 2026-08-27 (`logs/wildguard_copy.log`: `COPY_DONE`). **Copy, not move**;
the source project is untouched at 14G.

**The check that mattered:** the copy is snapshot `cbba4823f3e8020e5a74a5e29bf85072def6f2ff`, and that
SHA appears **1,240 times** in this project's job logs as the judge every recorded result was produced
with. Same weights, so every future WildGuard number stays comparable with every recorded one. Had it
differed, the licence would have been the smaller problem.

Preflight now reports `DRY RUN OK` on all twelve checks. No credential was handled at any point.
