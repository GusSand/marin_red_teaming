...
| `IN-007` | CRITICAL | Gus | **Restore WildGuard access.** I destroyed the workspace HF cache with an `rsync --delete` on 2026-09-08 (details in the journal). Public weights are re-downloading and everything else is recovered, but `allenai/wildguard` is gated and returns 401 from Torch with no token on the machine. Every judged run is blocked until a token with that repo's licence accepted is available to jobs — or until you point me at the copy the 2026-08-27 `logs/wildguard_copy.log` came from. | `S1-PREFIX`, `S1-06`, and every future harmbench run | A WildGuard load succeeding from the safety-eval environment; `scripts/dry_run_check.py` reporting `judge cached (offline)` OK. |
<!-- ACTIVE_INBOX_END -->
