...
| `build_gpt_rater_package.py` | `S1-ENDORSE-V2`: the GPT second-rater package — frozen shards in three formats, fill-in sheets in the locked column schema, output contract, and hashes. Reuses the `f####` ids deliberately so rater B joins one-to-one onto sealed rater A. Emits no `key.json`. |
| `check_gpt_rater_package.py` | `S1-ENDORSE-V2`: verifies a built rater package before handover — no key material, item formats agree, sheet header parses under the converter schema, and no checkpoint token in `upload/` or anywhere else. |
| `sheet_to_labels.py` | `S1-ENDORSE-V2`: converts a returned flat rater sheet into the label JSONL the validator consumes. `--selftest` round-trips a real label file through the sheet schema and asserts the canonical form survives. |
