...
| `S1-PREFIX` | BLOCKED | Research agent | Test whether Starling's delivery framing is proximally sufficient to move Phoenix's WildGuard harmful rate. Preregistration, scaffolds, sbatch and preflight are frozen and staged. | Blocked on `OPS-001`: the judge weights are gone and gated. Submit the moment `IN-007` resolves; nothing about the design changes. | `docs/experiments/09-08_prefix_framing-intervention.md` |
| `OPS-001` | IN_PROGRESS | Research agent | Recover the Torch workspace after an `rsync --delete` destroyed everything the repo does not track. | safety-eval tree restored from its own `.git`; `pythons/` reinstalled and the venv verified (3.12.14 / torch 2.8.0 / vllm 0.11.0); twins v2 raw generations preserved outside the workspace; marin weights re-downloading. Remaining: WildGuard, which needs `IN-007`. | `docs/research_journal.md` 2026-09-08 incident entry; `scripts/sync_to_torch.sh` |
