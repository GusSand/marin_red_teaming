...
| `S1-JUDGE-VOCAB` | IN_PROGRESS | Research agent | Audit every local judge output for stance, task and relevance values outside the locked rubric vocabulary, and for quality-null rule violations. | Promoted from PARKED: it is the only unblocked work while `IN-007` holds. CPU-only on files already in hand. Scope frozen before looking. Cannot rescue the 08-29 selection verdict either way. | `docs/experiments/09-09_judge-vocabulary-audit.md`; `docs/experiments/08-31_spotcheck_anchor-audit.md` |
