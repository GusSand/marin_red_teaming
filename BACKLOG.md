...
| `S1-05` | DONE | Research agent | NOT EVALUABLE: the all-four composite floored (0.00% / 1.85%) below the 5% gate, because exact paragraph equality is near-unpassable for a base model. | Closed. Benign task success is unmeasured, not flat; no stance-specific claim available. Re-run scoped as `S1-05B`. | `docs/experiments/09-07_benign-twins_control.md`; `docs/results/09-07_benign_twins/` |
| `S1-05B` | READY | Research agent | Re-run the benign-twins control with a gradable constraint set. | Replace exact paragraph equality with a tolerance, define the body independently of the title check, and replace the bare substring audience check. Reuse the twins, generator and job. | `docs/experiments/09-07_benign-twins_control.md` |
| `S1-CKPT` | IN_PROGRESS | Research agent | Localize the Phoenix→Starling behavior change using three public cooldown revisions. | Next after `S1-05`: freeze the evaluation protocol; pre-download and verify steps 1,340,000, 1,360,000 and 1,380,000 by full SHA; reuse the trajectory harness. No David checkpoint gate. | `docs/planning/09-07_public-cooldown-checkpoints.md`; `outputs/2026-09-07_cooldown-config-audit/checkpoint-evidence.json` |
...
| `S1-SYNTH` | BLOCKED | Research agent | Produce the Stage 1 verdict and frozen Stage 2 endpoints. | Task-dependency blocked on `S1-05B` and `S1-06`. Incorporate the config audit now and `S1-CKPT` timing results once independently verified; timing is descriptive, not causal. | `STATUS.md` Stage 1 exit criteria |
