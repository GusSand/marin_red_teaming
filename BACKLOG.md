...
- [~] **3. Behavior-level distribution decomposition** phoenix→starling, 1,080 existing responses, six categories, mass changes with behavior bootstrap. Never phrased as flows. **Pass-2 annotation running 2026-08-29 (4 blind Fable instances on balanced shards, 135/135 each).** Analysis path smoke-tested on synthetic labels (null in, null out). Test-retest rules pre-registered. Next: merge_sheets → decompose_distribution → retest_agreement → fresh-subagent verification.
