...
| `S1-05` | IN_PROGRESS | Research agent | Determine whether the compliance change appears on matched benign instructions. | Twins and grader built and smoke-tested. Resuming: write the sbatch, preflight, submit the 324-generation job. | `docs/experiments/09-07_benign-twins_control.md` |
| `S1-CKPT` | READY | Research agent | Localize the Phoenix→Starling behavior change using three public cooldown revisions. | Next after `S1-05`: freeze the evaluation protocol; pre-download and verify steps 1,340,000, 1,360,000 and 1,380,000 by full SHA; reuse the trajectory harness. No David checkpoint gate. | `docs/planning/09-07_public-cooldown-checkpoints.md`; `outputs/2026-09-07_cooldown-config-audit/checkpoint-evidence.json` |
...
| `S1-SYNTH` | BLOCKED | Research agent | Produce the Stage 1 verdict and frozen Stage 2 endpoints. | Task-dependency blocked on `S1-05` and `S1-06`. Incorporate the config audit now and `S1-CKPT` timing results once independently verified; timing is descriptive, not causal. | `STATUS.md` Stage 1 exit criteria |
