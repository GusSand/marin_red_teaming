...
| `S1-3F` | BLOCKED | Research agent | Primary VERIFIED: MAINLY UNQUALIFIED, 68.2% of the endorsement-mass increase (+19.07pp) vs concessionary 31.8%. Misclassified material in both arms, as a levels caveat. | Blocked on `IN-004`, the second-rater slice. The concessionary/misclassified boundary is unmeasured until it returns. | `docs/experiments/09-04_phoenix-starling_concessionary-endorsement.md`; `docs/results/09-04_concessionary/` |
...
| `S1-SYNTH` | BLOCKED | Research agent | Produce the Stage 1 verdict and frozen Stage 2 endpoints. | Task-dependency blocked on `S1-3F`, `S1-05` and `S1-06`. No person needs to act, so no INBOX ID. | `STATUS.md` Stage 1 exit criteria |
