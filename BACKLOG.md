...
| `S1-FORMAT` | IN_PROGRESS | Research agent | Test whether the document-persona difference found in the benign twins also appears in the 1,080 labelled misinformation responses. | Preregistration frozen; running the CPU-only prevalence analysis on `full_phoenix_starling_v1`, then independent verification. | `docs/experiments/09-08_format-carryover.md` |
| `S1-05C` | READY | Research agent | Benign control on constraints orthogonal to document format, so no two are satisfiable by one emitted line. | Freeze a preregistration using externally-authored verifiable instructions on the 54 matched twin subjects, plus an inter-constraint correlation gate checked before the primary is read. Fresh generations; do not re-grade v2. Derive the echo marker from the prompt at build time. | `docs/experiments/09-08_benign-twins_v2.md` |
...
| `S1-SYNTH` | BLOCKED | Research agent | Produce the Stage 1 verdict and frozen Stage 2 endpoints. | Task-dependency blocked on `S1-FORMAT`, `S1-05C` and `S1-06`. Carry S1-05 as NOT EVALUABLE, with no generalization or stance-specific inference. Carry the verified timing bound as descriptive, not causal. | `STATUS.md` Stage 1 exit criteria |
