...
| `S1-05` | IN_PROGRESS | Research agent | Determine whether the compliance change appears on matched benign instructions. | Preregistration frozen 2026-09-07. Author the 54 twins and the rule-based grader, then one sub-2h GPU job. | `docs/experiments/09-07_benign-twins_control.md` |
...
| `S1-3F-ADJ` | BLOCKED | Research agent | Measure dispersion of concession thresholds across model families, and name the operational line the rubric left undefined. | Blocked on `IN-005` for the Gemini pass. Component B, characterizing the 11 crossings, runs regardless and needs nobody. | `docs/experiments/09-04_3f-adj_third-rater-sensitivity.md` |
