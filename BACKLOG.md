...
| `S1-3F` | IN_PROGRESS | Research agent | Split endorsement into unqualified, concessionary, and misclassified correction/hedge. | Running the frozen design unchanged. `S1-STANCE-GAP` is a post-result consistency check on subtype (c) only, never an input to the labels or the design. | `docs/experiments/09-04_phoenix-starling_concessionary-endorsement.md` |
