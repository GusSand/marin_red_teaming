...
| `S1-STATS` | IN_PROGRESS | Research agent | The behaviour-level bootstrap used across this project conditions on the seed draw and is ~6x anticonservative: on 126 disjoint 5-vs-5 splits of phoenix against itself, its CI excludes 0 in 27.8% of splits (refusal 34.9%) against a nominal 5%. | Freeze a corrected inference procedure (seed as a unit, or a nested behaviour x seed resample), then re-derive the published intervals for every recorded contrast. **`S1-FORMAT`'s "well-powered null" and its 4.34pp MDE are the first things to re-check** — a null argued from a narrow interval is exactly what this defect breaks. Directions of the large effects survive; small effects and nulls do not automatically. | `docs/experiments/09-09_inference-procedure_recalibration.md`; `scripts/calibrate_behavior_bootstrap.py`; `docs/experiments/09-08_prefix_framing-intervention.md` |
...
| `S1-SYNTH` | BLOCKED | Research agent | Produce the Stage 1 verdict and frozen Stage 2 endpoints. | Task-dependency blocked on `S1-05C`, `S1-06` and `S1-STATS`. The prefix-intervention result is in: carry its **narrow** claim only — a refusal-suppressing prefill is proximally sufficient, and this framing is not shown to be the mechanism. `S1-STATS` gates the intervals every Stage 1 verdict is stated with. Carry S1-05 as NOT EVALUABLE, with no generalization or stance-specific inference. Carry the verified timing bound as descriptive, not causal. | `STATUS.md` Stage 1 exit criteria |
