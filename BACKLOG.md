...
| `S1-05B` | IN_PROGRESS | Research agent | Re-run the benign-twins control with a gradable constraint set. | After `S1-CKPT`: check component feasibility on separate calibration examples, then freeze a new preregistration. Use paragraph tolerance, an independent body definition and a real audience check. Reuse the twins, generator and job; retain the original NOT EVALUABLE result. | `docs/experiments/09-07_benign-twins_control.md` |
