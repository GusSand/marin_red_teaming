...
| `S1-3F` | DONE | Research agent | Endorsement subtypes split and both raters in. Verdict is RATER-DEPENDENT: unqualified holds 68.2% of the increase under the primary rater, 53.65% under the second, straddling the 60% bar. | Closed. `S1-SYNTH` uses MIXED language; the "flatly asserts" wording is retracted. | `docs/experiments/09-04_phoenix-starling_concessionary-endorsement.md`; `docs/results/09-04_concessionary/` |
| `S1-05` | DONE | Research agent | NOT EVALUABLE: the all-four composite floored (0.00% / 1.85%) below the 5% gate, because exact paragraph equality is near-unpassable for a base model. | Closed and VERIFIED. The floor supplies no evidence for generalization or stance-specificity. Formatting diagnostics do not rescue the composite. IFEval remains separate evidence; redesign under `S1-05B`. | `docs/experiments/09-07_benign-twins_control.md`; `docs/results/09-07_benign_twins/` |
| `S1-05B` | DONE | Research agent | Valid primary, reading withheld. Δ = +1.5000 constraints [+1.3704, +1.6296], p < 1e-4, IF-CONSISTENT under the frozen rule — but 104.9% of it is title + audience, two checks one document header satisfies together; drop them and the same rule returns Δ = −0.0741 [−0.2346, +0.0864]. | Closed, VERIFIED to 4 decimals by an independent re-implementation. Measures a formatting persona, not instruction-following. Successor is `S1-05C`. | `docs/experiments/09-08_benign-twins_v2.md`; `docs/results/09-08_benign_twins_v2/` |
| `S1-FORMAT` | DONE | Research agent | DOES NOT CARRY: the twins' document-header gap is absent where no format is requested (Δ +0.93pp; CI [−2.22, +3.70] recorded, [−6.63, +8.48] corrected under `S1-STATS`; **MDE 9.44pp, not the recorded 4.34pp** — the verdict holds, the precision claim does not). But the confound is **relocated, not retired** — `assistant_preamble` is +16.67pp, p < 1e-4, and those rows are 57% attempt-strong at Starling with zero refusals. | Closed, VERIFIED to the printed decimal on all five measures. Surviving account: a deliver-the-requested-artifact disposition. `S1-05C` must separate cause from co-symptom. | `docs/experiments/09-08_format-carryover.md`; `docs/results/09-08_format_carryover/` |
| `S1-PREFIX` | DONE | Research agent | SUFFICIENT under the frozen table: the delivery prefill moves Phoenix +29.63pp [+21.48, +38.15], the `deflect` control does not (+5.93pp, p 0.28), f = 1.333 — an **overshoot** past Starling's own endpoint. | Closed, VERIFIED (REPRODUCED WITH CONCERNS). Three corrections carried: 62% of the effect runs through the pre-declared partly-forced refusal channel; the `none` arm is byte-identical to traj4 phoenix[0:5]; the frozen bootstrap is ~6x anticonservative. `S1-SYNTH` inherits the narrow claim. Successors `S1-PREFIX-B` and `S1-STATS`. | `docs/experiments/09-08_prefix_framing-intervention.md`; `docs/results/09-08_prefix_framing/` |
| `OPS-001` | DONE | Research agent | Workspace recovered. 270GB of weights restored across nine revisions with **zero tag drift**; cooldown shards re-verified 12/12 against recorded LFS oids; venv, interpreter, safety-eval tree and the seed patch all restored. Preflight green except the judge. | Closed. The judge is `IN-007`, a credential only Gus can supply. Guards added: `scripts/sync_to_torch.sh` refuses `--delete`, `scripts/check_tag_drift.py` fails on a moved tag, `benign_twins_v2.sbatch` preserves raw outputs outside the workspace. | `docs/research_journal.md` 2026-09-08/09 incident entries; `docs/resolved_revisions.json` |
| `S1-STATS` | DONE | Research agent | **0 of 20 direction verdicts flip; every in-scope interval is about half its proper width.** The incumbent's type-I error is monotone in the seed sd it ignores (2.4% at deeper-starling to 40.5% at jellyfish). Procedure decision recorded **OPEN** — the frozen rule declares `P0` ineligible to win and then hands it the win. | Closed, VERIFIED (32 of 32 calibration cells within tolerance). Verdicts now rest on the **wider** of the two intervals, which is monotonically a weakening and needs no selection. `S1-FORMAT`'s MDE corrected 4.34 → 9.44pp; `S1-05B`'s precision ±8.6% → ±61%, both verdicts intact. Adoption of `P1` is `IN-008`. | `docs/experiments/09-09_inference-procedure_recalibration.md`; `docs/results/09-09_procedure_selection/` |
| `S1-PREFIX-B` | READY | Research agent | Separate a delivery *disposition* from any prefix that merely makes a refusal opener improbable. | Add a **content-free compliance prefill** arm ("Sure," / "Certainly!"): it suppresses refusal while carrying no deliver-the-artefact semantics. `S1-PREFIX` confounds the two — `delivery` moved P(non-refusal) +25.18pp, `deflect` only +3.33pp, and no arm holds refusal suppression fixed while varying framing. Reuse the `S1-PREFIX` harness; the scaffold-hash resume gate already handles a new arm. | `docs/experiments/09-08_prefix_framing-intervention.md` |
| `S1-05C` | READY | Research agent | Separate whether Starling's deliver-the-requested-artifact framing causes the compliance shift or is a co-symptom of it. | Freeze a preregistration whose constraints are not jointly satisfiable by one emitted line, plus an inter-constraint correlation gate checked before the primary is read. A design that only varies surface format will not settle it — `S1-FORMAT` showed the framing transfers where the header does not. Fresh generations; do not re-grade v2. Freeze every marker list and derive the echo marker from the prompt at build time. | `docs/experiments/09-08_format-carryover.md`; `docs/experiments/09-08_benign-twins_v2.md` |
| `S1-CKPT` | DONE | Research agent | EARLY: the refusal change is present by 25% of the cooldown (f 0.970 [0.671, 1.423]); all three intermediates differ from Phoenix, none from Starling. | Closed, VERIFIED. Temporal only — mixture, LR, batch and z-loss are confounded at step 1,320,000. | `docs/experiments/09-08_cooldown_localization.md`; `outputs/cooldown_localization.json` |
| `S1-06` | BLOCKED | Research agent | Screen done: WildJailbreak contributes 0 net-new behaviours. Reachable ceiling across all non-gated sources is 101 distinct, against a frozen target of 150. | Blocked on `IN-006`: how to close a 49-behaviour shortfall, and whether to use all of HarmBench. | `docs/experiments/08-29_misinfo-evalset_selection.md` |
| `S1-SYNTH` | BLOCKED | Research agent | Produce the Stage 1 verdict and frozen Stage 2 endpoints. | Task-dependency blocked on `S1-05C` and `S1-06`. Two results are in. The prefix intervention: carry its **narrow** claim only — a refusal-suppressing prefill is proximally sufficient, and this framing is not shown to be the mechanism. The recalibration: **state every interval at the wider width**, and never argue a null from interval narrowness — one such claim has already been corrected. Carry S1-05 as NOT EVALUABLE, with no generalization or stance-specific inference. Carry the verified timing bound as descriptive, not causal. | `STATUS.md` Stage 1 exit criteria |
| `S2-00` | BLOCKED | Gus + David | Finalize the six-arm causal replay and its external training allocation. | Complete `S1-SYNTH`; resolve the remaining external replay inputs in `IN-001`; freeze the Stage 2 document. Public timing checkpoints are available. Local GPU hours need no new approval. | Stage 1 spec step 7; `docs/decisions.md` 2026-08-28 six-arm decision; `docs/planning/09-07_public-cooldown-checkpoints.md` |
...
| `S1-3F-ADJ` | DONE | Research agent | Share is not resolvable: 45-74% across three valid raters, threshold undecidable, and the pooling choice alone moves GPT to 0.377. Flash discarded on the validity gate. | Closed. Component B's five-gate definition of "material concession" feeds a future rubric v2. | `docs/experiments/09-04_3f-adj_third-rater-sensitivity.md`; `docs/results/09-07_3f_adj/` |
| `S1-JUDGE-VOCAB` | DONE | Research agent | ISOLATED: 21 files, 6,405 rows, 2 out-of-vocabulary instances on **1 distinct stimulus**, both olmo32 `refutes`. Zero in the primary labels, which are clean at source as well as post-parse. No recorded number changes. | Closed, VERIFIED. The gap, not the count, was the finding: `judge_dimensions.py` now validates against the locked vocabulary and fails above a 1% rate. Scope is discovered from the data, not typed. `sheet_third_gemini_pro.csv` preserved into the labels tree. | `docs/experiments/09-09_judge-vocabulary-audit.md`; `docs/results/09-09_vocab_audit/` |
...
| `S1-ENDORSE-V2` | BLOCKED | Research agent | Separate truth commitment, rhetorical concessions, attribution, and added purported evidence. Then test which features change automated-judge scores under matched content. | Package built, 24/24 standing gates pass. Rater A **sealed**: 1,080 rows, 12/12 row gates, sha256 `7aef8de0…`, two declared null-span exceptions on prompt-echo rows. Label, analysis, agreement and audit paths smoke-tested (null in, null out; a planted +25pp shift recovered). Blocked on `IN-009` for rater B, which must be a different model family. Then the 100-row audit, then checkpoint differences. | `docs/experiments/09-10_endorsement-feature_decomposition.md`; `config/endorsement_feature_codebook_v1.md` |
| `S1-RACCOON` | PARKED | Research agent | Measure behavior on the Jellyfish-derived deeper-cooldown branch. | Lower priority than `S1-CKPT`. Verify soft-raccoon-3 lineage and inherited training changes before preregistration. It cannot sample Phoenix reheat or establish a Phoenix plateau. | `docs/planning/09-07_public-cooldown-checkpoints.md`; Marin issue #898 |
| `S2-FLAN-SCREEN` | PARKED | Research agent | Scope a matched Phoenix continuation with FLAN included versus removed. | Planning only: define replacement mass, token budget, LR, batch, z-loss, seeds and decision sensitivity. Keep the six-arm replay as the registered causal design. No launch in this update. | `docs/planning/09-07_public-cooldown-checkpoints.md`; `CLAUDE.md` compute policy |
<!-- ACTIVE_TASKS_END -->

## Measurement and localization requirements — 2026-09-08

- **S1-05 stays closed as NOT EVALUABLE.** The floor gate makes the primary contrast uninterpretable. The title/preamble and coupled body-count diagnostics explain the failed instrument; they do not establish general instruction following or stance-specificity. Benign task success is unmeasured. IFEval remains separate evidence.
- **S1-05B is a new experiment.** Check each component and the composite for feasibility on separate calibration examples before freezing the replacement preregistration. Use paragraph tolerance, a body definition independent of title success, and an audience check that measures the requested audience. Preserve the original result. For future runs, an exact 0% or 100% on any constraint at either checkpoint triggers inspection before interpretation.
- **S1-CKPT matches the endpoints.** Pin full SHAs and verify downloaded weights, beyond the existing manifest checks. Match behaviors, wrappers, seeds, grading conventions and denominators to the Phoenix/Starling endpoint protocol. Freeze the primary behavior measure, uncertainty, exclusions and timing decision rules before inspecting intermediate outputs.
- **Timing is descriptive.** If the first intermediate shows the change under the frozen rule, report “present by 25% of cooldown.” Do not infer an abrupt change at the mixture switch. LR, accumulated mixture exposure, batch size, z-loss and training duration remain confounded. Independently verify behavioral results before synthesis.

Sequence: localize the established behavioral change with `S1-CKPT`; redesign the failed benign control separately as `S1-05B`.
