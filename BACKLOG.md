...
| `S1-STANCE-GAP` | PARKED | Research agent | Quantify how pass-2 and the two calibration anchors labelled restatement-without-assertion items, which had no stance convention until 2026-08-31. | Run only after gs157's 25-item spot-check is submitted, so it cannot steer the audit. Feeds `S1-3F` subtype (c). | `config/annotator_conventions_v1.md` rule 6 and its scope note |
