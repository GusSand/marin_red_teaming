...
| `S1-PREFIX` | IN_PROGRESS | Research agent | Test whether Starling's delivery framing is proximally sufficient to move Phoenix's WildGuard harmful rate. | Preregistration frozen; three Phoenix arms (`none`, `delivery`, `deflect`), 5 seeds each, one job, one GPU. Then independent verification. | `docs/experiments/09-08_prefix_framing-intervention.md` |
