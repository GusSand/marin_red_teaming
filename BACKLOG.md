...
| `S1-PREFIX` | IN_PROGRESS | Research agent | Test whether Starling's delivery framing is proximally sufficient to move Phoenix's WildGuard harmful rate. | `IN-007` resolved with no token: the judge was restored from the safety-decay project at the identical snapshot `cbba4823`, confirmed against 1,240 log references. Preflight `DRY RUN OK`. Submitting the three-arm job; then independent verification. | `docs/experiments/09-08_prefix_framing-intervention.md` |
...
| `S1-SYNTH` | BLOCKED | Research agent | Produce the Stage 1 verdict and frozen Stage 2 endpoints. | Task-dependency blocked on `S1-PREFIX`, `S1-05C` and `S1-06`. Carry S1-05 as NOT EVALUABLE, with no generalization or stance-specific inference. Carry the verified timing bound as descriptive, not causal. | `STATUS.md` Stage 1 exit criteria |
