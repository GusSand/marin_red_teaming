...
| `S1-PREFIX` | BLOCKED | Research agent | Test whether Starling's delivery framing is proximally sufficient to move Phoenix's WildGuard harmful rate. Preregistration, scaffolds, sbatch and preflight are frozen and staged. | Blocked on `IN-007`: the judge weights are gone and gated. Everything else is restored and the preflight is green. Submit the moment `IN-007` resolves; nothing about the design changes. | `docs/experiments/09-08_prefix_framing-intervention.md` |
