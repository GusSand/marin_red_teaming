...
| `S1-05B` | READY | Research agent | Re-run the benign-twins control with a gradable constraint set. | After `S1-CKPT`: check component feasibility on separate calibration examples, then freeze a new preregistration. Use paragraph tolerance, an independent body definition and a real audience check. Reuse the twins, generator and job; retain the original NOT EVALUABLE result. | `docs/experiments/09-07_benign-twins_control.md` |
| `S1-CKPT` | IN_PROGRESS | Research agent | Localize the Phoenix→Starling behavior change using three public cooldown revisions. | Current: freeze the primary behavior measure and timing rules before viewing outcomes. Match the endpoint protocol; pin full SHAs and verify downloaded weights for 1,340,000 / 1,360,000 / 1,380,000. Apply the interpretation limits below. No David gate. | `docs/planning/09-07_public-cooldown-checkpoints.md`; `outputs/2026-09-07_cooldown-config-audit/checkpoint-evidence.json` |
...
| `S1-SYNTH` | BLOCKED | Research agent | Produce the Stage 1 verdict and frozen Stage 2 endpoints. | Task-dependency blocked on `S1-05B` and `S1-06`. Carry S1-05 as NOT EVALUABLE, with no generalization or stance-specific inference. Incorporate `S1-CKPT` only after independent verification; timing is descriptive, not causal. | `STATUS.md` Stage 1 exit criteria |
