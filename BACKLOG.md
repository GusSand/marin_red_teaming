...
| `S1-ENDORSE-V2` | IN_PROGRESS | Research agent | Separate truth commitment, rhetorical concessions, attribution, and added purported evidence. Then test which features change automated-judge scores under matched content. | Package built, 24/24 standing gates pass. Label, analysis, agreement and audit paths all smoke-tested against synthetic labels (null in, null out; a planted +25pp shift recovered). Rater A running on the full set; rater B is `IN-009`, and needs a different model family. Seal both, then the 100-row audit, then checkpoint differences. | `docs/experiments/09-10_endorsement-feature_decomposition.md`; `config/endorsement_feature_codebook_v1.md` |
