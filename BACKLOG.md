...
| `S1-STATS` | READY | Research agent | The behaviour-level bootstrap used across this project conditions on the seed draw and is ~6x anticonservative: on 126 disjoint 5-vs-5 splits of phoenix against itself, its CI excludes 0 in 27.8% of splits (refusal 34.9%) against a nominal 5%. | Freeze a corrected inference procedure (seed as a unit, or a nested behaviour x seed resample), then re-derive the published intervals for every recorded contrast. **`S1-FORMAT`'s "well-powered null" and its 4.34pp MDE are the first things to re-check** — a null argued from a narrow interval is exactly what this defect breaks. Directions of the large effects survive; small effects and nulls do not automatically. | `scripts/calibrate_behavior_bootstrap.py`; `docs/experiments/09-08_prefix_framing-intervention.md` |
